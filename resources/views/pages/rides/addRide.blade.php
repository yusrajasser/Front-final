...
@section('title')
    <title>لوحة التحكم | إضافة رحلة</title>
@endsection
...
@section('content')
    ...
    <form method="POST" class="d-flex flex-column justify-content-center align-items-center"
        action="{{ route('ride.store') }}" dir="rtl">
        @csrf
        <h1 class="h3 mb-3 fw-normal p-5 rounded"> إضافة رحلة</h1>

        <input type="hidden" name="driver_id" value="{{ $data['driver_id'] }}">
        {{-- <input type="hidden" name="actual_reservation_nums" value="{{ $data['cars']->seats_num }}"> --}}
        <div class="d-flex justify-content-center gap-1 col-6 pt-2">
            <div class="col-6">
                <label class="col-sm-4" for="from">من</label>
                <input class="form-control col-12" type="text" name="from" required>
                @error('from')
            ...
            <div class="col-6">
                ...
                <input class="form-control col-12" type="text" name="to" required>
                @error('to')
                    <div class="text-danger">
                        {{ $message }}
                    </div>
        ...
        <div class="form-row p-1 rounded col-6">
            ...
            <div class="col-12">
                <input class="form-control col-md-4" type="time" name="time" step="600" required>
            </div>
            @error('time')
                <div class="text-danger">
                    {{ $message }}
        ...
        {{-- <div class="form-row p-1 rounded col-6">
            ...
            <input class="form-control col-sm-8" type="number" step="0.1" min="0" name="amount" required>
            @error('amount')
                <div class="text-danger">
                    {{ $message }}
                </div>
        ...
        <div class="d-flex flex-justify-center pt-5">
            <button class="w-100 btn btn-lg btn-primary" type="submit">حفظ</button>
